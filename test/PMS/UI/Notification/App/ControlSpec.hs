...
module PMS.UI.Notification.App.ControlSpec (spec) where
...
import qualified PMS.UI.Notification.App.Control as SUT
import qualified PMS.UI.Notification.DM.Type as SUT
  ...
  describe "runWithAppData" $ do
    context "when McpToolListChangedNotification" $ do
      it "should be JsonRpcNotification" $ \ctx -> do 
        ...
        STM.atomically $ STM.writeTQueue queue noti

        jsonStr <- hGetLine readH

        let actual = "notifications/tools/list_changed" `isInfixOf` jsonStr
